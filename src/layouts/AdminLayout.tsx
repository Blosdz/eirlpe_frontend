...
import { LogOut, Users, Globe, LayoutDashboard, Layout } from "lucide-react";
...
export default function AdminLayout() {
  ...
  const navigation = [
    ...
    { name: "Hostnames", href: "/admin/hostnames", icon: Globe },
    { name: "Templates", href: "/admin/templates", icon: Layout },
  ];

  return (
    <div className="relative z-10 min-h-screen bg-background-light dark:bg-background-dark font-display flex flex-col md:flex-row shadow-sm">
      {/* Sidebar */}
      <aside className="w-full md:w-64 bg-white dark:bg-surface-dark border-r border-charcoal/10 dark:border-primary/20 flex flex-col md:min-h-screen">
        <div className="p-6 border-b border-charcoal/10 dark:border-primary/20">
          <h1 className="text-2xl font-serif text-charcoal dark:text-primary">
            EIRL.pe{" "}
            <span className="text-sm font-sans block text-charcoal/50 dark:text-primary/60">
              Admin Panel
            </span>
          </h1>
        </div>
