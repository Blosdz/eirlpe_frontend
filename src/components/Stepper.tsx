        ...
        return (
          <div key={step.label} className="flex items-center">
            <div className="flex flex-col items-center gap-1 sm:gap-1.5 md:gap-2">
              <div
                className={`flex items-center justify-center size-8 sm:size-9 md:size-10 lg:size-11 rounded-full border-2 transition-all duration-300 shrink-0 ${
                  isDone
              ...
              <span
                className={`text-[9px] sm:text-[10px] md:text-xs font-bold tracking-[0.08em] sm:tracking-[0.1em] uppercase transition-colors duration-300 whitespace-nowrap ${
                  isDone || isCurrent
            ...
            {!isLast && (
              <div
                className={`w-6 sm:w-10 md:w-14 lg:w-20 h-px mb-4 sm:mb-5 md:mb-6 mx-1 sm:mx-2 md:mx-3 transition-colors duration-300 shrink-0 ${
                  isDone
                    ? "bg-accent dark:bg-accent-light"
                    : "bg-charcoal/15 dark:bg-primary/20"
                }`}
              />
            )}
          </div>
