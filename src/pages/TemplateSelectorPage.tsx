    ...
    return (
        <button
            ...
            className={`group text-left rounded-2xl sm:rounded-3xl overflow-hidden border-2 transition-all duration-300 ${
                selected
                    ? 'border-charcoal dark:border-primary ring-2 ring-charcoal/20 dark:ring-primary/30 scale-[1.02]'
                    : 'border-charcoal/10 dark:border-primary/15 hover:border-charcoal/30 dark:hover:border-primary/30 hover:shadow-lg'
            }`}
        >
            {/* Preview */}
            <div className={`relative aspect-[4/3] bg-gradient-to-br ${template.preview} p-6 sm:p-8 flex flex-col justify-between`}>
                ...
                {/* Checkmark */}
                {selected && (
                    <div className="absolute top-4 right-4 size-8 sm:size-10 rounded-full bg-charcoal dark:bg-primary text-primary dark:text-charcoal flex items-center justify-center">
                        <span className="material-symbols-outlined text-lg sm:text-xl">check</span>
                    </div>
                )}
            </div>
            {/* Info */}
            <div className="p-5 sm:p-6 bg-white dark:bg-charcoal/50 space-y-2 sm:space-y-3">
                <div className="flex items-center justify-between">
                    <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-charcoal dark:text-primary tracking-tight">{template.name}</h3>
                    <span className="text-[10px] sm:text-xs font-bold tracking-[0.15em] uppercase text-muted-beige dark:text-primary/50 border border-charcoal/10 dark:border-primary/15 px-2.5 py-1 rounded-full">
                        {template.category}
                    </span>
                </div>
                <p className="text-sm sm:text-base text-muted-beige dark:text-primary/60 leading-relaxed">{template.description}</p>
            </div>
        </button>
    );
}
            ...
            <main className="pt-28 sm:pt-32 md:pt-36 pb-20 sm:pb-24 px-6 sm:px-8 md:px-12 lg:px-20">
                <div className="mx-auto max-w-7xl">
                    ...
                    <div className="text-center space-y-4 sm:space-y-6 mb-12 sm:mb-16 lg:mb-20">
                        ...
                        <h1 className="font-serif text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-charcoal dark:text-primary leading-tight">
                            Elige tu <span className="italic font-normal">template</span>
                        </h1>
                        <p className="text-lg sm:text-xl md:text-2xl text-muted-beige dark:text-primary/60 font-light max-w-2xl mx-auto">
                            Selecciona el diseño para <span className="font-bold text-charcoal dark:text-primary">{domainDisplay}</span>
                        </p>
                    </div>
                    ...
                    <div className={`fixed bottom-0 left-0 right-0 z-40 transition-all duration-500 ease-out ${
                        ...
                        <div className="bg-charcoal dark:bg-primary border-t border-charcoal/20 dark:border-primary/20 px-6 sm:px-8 md:px-12 lg:px-20 py-4 sm:py-5">
                            <div className="mx-auto max-w-7xl flex items-center justify-between gap-4">
                                <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                                    <span className="text-primary/70 dark:text-charcoal/70 text-sm sm:text-base font-light truncate">
                                        {domainDisplay}
                                    </span>
                                    <span className="text-primary/30 dark:text-charcoal/30 hidden sm:inline">/</span>
                                    <span className="text-primary dark:text-charcoal text-sm sm:text-base font-bold hidden sm:inline">
                                        {TEMPLATES.find(t => t.id === selectedTemplate)?.name}
                                    </span>
                                </div>
                                <button
                                    onClick={() => navigate(`/registro/${encodeURIComponent(domain || '')}/${encodeURIComponent(selectedTemplate || '')}`)}
                                    className="shrink-0 px-6 sm:px-8 md:px-10 py-3 sm:py-3.5 bg-primary dark:bg-charcoal text-charcoal dark:text-primary rounded-full font-bold text-sm sm:text-base tracking-wider uppercase hover:opacity-90 transition-all duration-300 active:scale-95"
                                >
                                    Continuar
                                </button>
                            </div>
